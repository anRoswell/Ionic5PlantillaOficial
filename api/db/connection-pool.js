...
function ConnectionPool(poolConfig, connectionConfig) {
	...
	this.idleTimeout =
		!poolConfig.idleTimeout && poolConfig.idleTimeout !== false
			? 300000 //5 min
			: poolConfig.idleTimeout

	this.retryDelay = !poolConfig.retryDelay && poolConfig.retryDelay !== false ? 5000 : poolConfig.retryDelay

	this.acquireTimeout =
		!poolConfig.acquireTimeout && poolConfig.acquireTimeout !== false
			? 60000 //1 min
			: poolConfig.acquireTimeout

	if (poolConfig.log) {
		if (Object.prototype.toString.call(poolConfig.log) == '[object Function]') this.log = poolConfig.log
		else {
...
function createConnection(pooled) {
	...
	var handleError = function (err) {
		...
		if (pooled.timeout) clearTimeout(pooled.timeout)
	...
	connection.on('connect', function (err) {
		...
		if (waiter !== undefined) setUsed.call(self, pooled, waiter)
		else setFree.call(self, pooled)
	})
...
function fill() {
	...
	var amount = Math.min(
		...
		this.waiting.length - available,
	) //how many are needed, minus how many are available
	...
	if (amount > 0) this.log('filling pool with ' + amount)

	for (i = 0; i < amount; i++) createConnection.call(this)
}
	...
	for (i = self.connections.length - 1; i >= 0; i--) {
		...
		if (pooled.con === connection) {
			...
			connection.reset(function (err) {
				if (!pooled.con)
					//the connection failed during the reset
					return

				if (err) {
					//there is an error, don't reuse the connection, just close it
					pooled.con.close()
					return
				}
...
ConnectionPool.prototype.drain = function (callback) {
	...
	if (eventTotal === 0 && callback) callback()

	var ended = function () {
		if (++eventCount === eventTotal && callback) callback()
	}
